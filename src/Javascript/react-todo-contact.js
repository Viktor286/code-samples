...
const filterOutList = (setList, setSettings, e) => {
  ...
  setList((list) =>
    list.map((el) => {
      const elCopy = { ...el };
      elCopy.visible =
        elCopy.name.toLowerCase().indexOf(filter) > -1 || elCopy.phone.toLowerCase().indexOf(filter) > -1;
      return elCopy;
    }),
  );
};
...
function ListItem({ listItem, setList }) {
  ...
  const removeContactHandler = useMemo(
    () => removeContact.bind(null, setList, listItem),
    [setList, listItem],
  );
  const toggleListElementHandler = useMemo(
    () => toggleListElement.bind(null, setList, listItem),
    [setList, listItem],
  );
  ...
  return (
    <>
      <li key={id} data-visible={visible}>
        <span data-active={active} onClick={toggleListElementHandler}>
          {name} | {phone}
        </span>
        <button onClick={removeContactHandler}>x</button>({count.current})
      </li>
    </>
  );
}
...
function ContactsApp({ savedContacts, savedSettings }) {
  ...
  const filterOutListHandler = useMemo(
    () => filterOutList.bind(null, setList, setSettings),
    [setList, setSettings],
  );
  const clearFilterHandler = useMemo(
    () => clearFilter.bind(null, setList, setSettings, _filter),
    [setList, setSettings, _filter],
  );
...
root.render(<ContactsApp {...{ savedSettings, savedContacts }} />);
